...
function formatRangeWithChunks(date1, date2, chunks, separator, isRTL) {
	...
	// Start at the leftmost side of the formatting string and continue utils you hit a token
	...
	segDragMousedown: function(seg, ev) {
		...
		var dragListener = new DragListener(view.coordMap, {
			...
			listenStart: function(ev) {
				mouseFollower.hide(); // don't show utils we know this is a real drag
...
$.extend(DayGrid.prototype, {
	...
	buildSegLevels: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// loop through levels, starting with the topmost, utils the segment doesn't collide with other segments
...
// Figure out which path forward (via seg.forwardSegs) results in the longest path utils
...
// who's width is unknown utils an edge has been hit. `seriesBackwardPressure` is the number of
...
function View(calendar) {
	...
	// Incrementing the current day utils it is no longer a hidden day, returning a copy.
